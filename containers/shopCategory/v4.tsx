...
export default function ShopCategory() {
  ...
  const uniqueSubCategories: Category[] = useMemo(() => {
    const subCategories = data?.data.children || [];
    if (!subCategories || !Array.isArray(subCategories) || subCategories.length === 0) {
      return [];
    }
    const categoryMap = new Map<Category['id'], Category>(
      subCategories.map((item: Category) => [item.id, item])
    );
    return Array.from(categoryMap.values());
  }, [data?.data.children]);

  console.log("ShopCategory - uniqueSubCategories:", uniqueSubCategories);
  console.log("ShopCategory - categoryLoading:", categoryLoading);
